using UnityEngine;

[CreateAssetMenu]
public class EnemyFactory : GameObjectFactory
{
    [SerializeField]
    private Enemy _prefab;

    [SerializeField, FloatRangeSliderAtribute(0.5f, 2f)]
    private FloatRange _scale = new FloatRange(1f);
    [SerializeField, FloatRangeSliderAtribute(-0.4f, 0.4f)]
    private FloatRange _pathOffset = new FloatRange(0f);

    public Enemy Get()
    {
        Enemy instance = CreateGameObjectInstance(_prefab);
        instance.OriginFactory = this;
        instance.Initialize(_scale.RandomValueInRange, _pathOffset.RandomValueInRange);
        return instance;
    }

    public void Reclaim(Enemy enemy)   // удаление врага
    {
        Destroy(enemy.gameObject);
    }
}

